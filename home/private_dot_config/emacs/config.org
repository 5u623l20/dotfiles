...
Enable auto-fill mode in text buffers
...
#+begin_src emacs-lisp
(setq user-mail-address "bofh@freebsd.org"
      user-full-name "Moin Rahman")
(setq gnus-select-method '(nnnil nil))
(setq gnus-secondary-select-methods
      '((nnimap "FreeBSD"
                (nnimap-address "imap.bofh.network")
                (mml-secure-openpgp-signers '("BDB9B5A617C0BC91"))
                (nnimap-server-port "imaps")
                (nnimap-stream ssl)
                (nnir-search-engine imap)
                (nnimap-expunge t)
                (nnmail-expiry-target "nnimap+FreeBSD:Trash")
                (nnmail-expiry-wait 'immediate))))
;; Reply to mails with matching email address
(setq send-mail-function 'smtpmail-send-it)
(setq smtpmail-stream-type 'starttls)
(setq smtpmail-smtp-server "smtp.bofh.network")
...
(setq gnus-posting-styles
      '((".*" ; Matches all groups of messages
         (address "bofh@freebsd.org"))
         (organization "The FreeBSD Project")
         ("X-Message-SMTP-Method" "smtp smtp.bofh.network 587 bsd")))
...
** Presentation with org-reveal
#+begin_src emacs-lisp
(use-package ox-reveal
  :after org
  :config
  ;; Ensure Reveal.js uses MathJax (needed for print-pdf mode)
  (setq org-reveal-mathjax t)
  ;; Enable PDF export (adds ?print-pdf compatibility)
  (setq org-reveal-pdf-fragment t)
  (setq org-reveal-root "https://cdn.jsdelivr.net/npm/reveal.js"))  ;; or local path
#+end_src
